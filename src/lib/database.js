...
/**
 * Get active prompt by key (for code integration)
 */
...
/**
 * Get prompts by category (for dropdowns and selection)
 */
...
export async function createPrompt(promptData) {
  try {
    console.log('🔧 [DATABASE] Creating prompt with data:', promptData);
    ...
    if (error) {
      console.error('🔧 [DATABASE] Supabase error details:', error);
      throw error;
    }
    
    console.log('🔧 [DATABASE] Prompt created successfully:', data);
    return data;
  } catch (error) {
 ...
 */
export async function togglePromptStatus(promptId, isActive) {
  try {
    const { data, error } = await supabase
      .from('prompts')
...
export const setPrimaryStoryCut = async (storyCutId, emberId, userId) => {
  try {
    const { data, error } = await supabase.rpc('set_story_cut_as_primary', {
      story_cut_id_param: storyCutId,
...
export const getPrimaryStoryCut = async (emberId) => {
  try {
    ...
    console.error('Error fetching primary story cut:', error);
    throw error;
  }
}; 